...
import org.jarachne.util.concurrent.JMXConfigurableThreadPoolExecutor;
...
//    }
    ...
    {
        Config c = Config.get();
        zooKeeper = new ZooKeeper(c.get("zk.quorum", "hadoop8:2181,hadoop7:2181,hadoop6:2181"), c.getInt("zk.session.timeout", 30 * 1000), defaultWatch);
    ...
    public void registerEphemeralNode(String path, String hostname)
        throws KeeperException, InterruptedException
    {
        ...
        String sb = new StringBuilder(path).append("/").append(hostname).toString();
        try
        {
		...
		ephemeralNodes.put(path, hostname);
...
//    }
	...
	public void addWatch(WatchBag bag)
	{
		...
		{
			throw new JarachneException("Unknown Watch Type," + watch.getClass() + ":" + watch.toString() + " path:" + bag.path);
		}
    ...
    {
		ZKClient.get().forceDelete("/s");
//		ZKClient.get().forceDelete("/S/search0");
		ZKClient.get().getZooKeeper().create("/aaa", "bb".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
    }
